...
<div class="container">
    ...
    <form action="{{ route('admin.pembayaran.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="penyewaan_id" id="penyewaan_id" class="form-select" required>
                ...
                @foreach ($penyewaan as $item)
                    <option value="{{ $item->id }}">{{ $item->kode_sewa }} - {{ $item->tanggal_sewa }}</option>
                @endforeach
            </select>
            @error('penyewaan_id')
        ...
        <div class="mb-3">
            ...
            <input type="number" name="jumlah_bayar" id="jumlah_bayar" class="form-control" value="{{ old('jumlah_bayar') }}" min="0" required>
            @error('jumlah_bayar')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
